...
def main():
    ...
    if uploaded_file is not None:
        ...
        # Debug için analyze_graduation_status() çıktısını ekrana yaz
        debug_result = analyze_graduation_status(transcript)
        st.write(f"🔍 analyze_graduation_status() çıktısı: {debug_result}")

        # Çıktı sayısını kontrol et
        if len(debug_result) != 6:
            st.error(f"❌ analyze_graduation_status() yanlış çıktı döndürüyor! Beklenen 6, alınan {len(debug_result)}")
        else:
            toplam_ects, ingilizce_ects, mesleki_secmeli_ects, secmeli_ects, başarısız_dersler, eksikler = debug_result
